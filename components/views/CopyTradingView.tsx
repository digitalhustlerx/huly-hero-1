...
import { Search, Filter, Shield, Zap, TrendingUp, Users, ArrowUpRight, ChevronDown, CheckCircle } from 'lucide-react';

const TraderProfileCard = ({ rank, name, pnl, winRate, risk, managed, followers, tags }: any) => (
  <div className="group relative bg-[#0e121a] border border-white/5 rounded-xl p-5 hover:border-blue-500/30 transition-all duration-300 hover:-translate-y-1">
    <div className="absolute top-4 right-4 text-xs font-mono text-gray-600 font-bold">#{rank}</div>
    ...
    <div className="grid grid-cols-2 gap-y-4 gap-x-2 mb-6">
      <div>
        ...
        <div className="text-xl font-bold font-mono text-green-400">+{pnl}%</div>
      </div>
      <div>
        <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Win Rate</div>
        <div className="text-xl font-bold font-mono text-white">{winRate}%</div>
      </div>
      <div>
        <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Money Managed</div>
        <div className="text-sm font-bold font-mono text-gray-300">{managed}</div>
      </div>
      <div>
        <div className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Risk Level</div>
        <div className={`text-sm font-bold font-mono px-2 py-0.5 inline-block rounded ${risk < 4 ? 'bg-green-500/20 text-green-400' : risk < 7 ? 'bg-yellow-500/20 text-yellow-400' : 'bg-red-500/20 text-red-400'}`}>
          {risk}/10
        </div>
      </div>
    </div>

    <button className="w-full py-3 bg-white/5 hover:bg-blue-600 text-white font-bold text-sm uppercase tracking-wider rounded border border-white/5 hover:border-blue-500 transition-all flex items-center justify-center gap-2">
    ...
    <div className="min-h-screen bg-[#030508] pb-20">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-end gap-8">
            ...
            <div className="flex gap-8 p-6 bg-[#0e121a]/80 backdrop-blur border border-white/10 rounded-xl">
               ...
               <div className="w-[1px] bg-white/10"></div>
               <div>
                 <div className="text-xs text-gray-500 uppercase tracking-wider mb-1">Avg. Monthly Profit</div>
                 <div className="text-2xl font-bold font-display text-green-400">+12.4%</div>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
             <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0">
                {['All Traders', 'Most Popular', 'Safe & Stable', 'High Profit', 'New Stars'].map(filter => (
                  <button 
                    key={filter} 
             ...
             <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="relative flex-grow sm:flex-grow-0">
                  ...
                  <input 
                    ...
                    className="w-full sm:w-64 pl-9 pr-4 py-2 bg-[#0e121a] border border-white/10 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500 transition-colors"
                  />
                </div>
                <button className="p-2 bg-[#0e121a] border border-white/10 rounded-lg text-gray-400 hover:text-white transition-colors">
                   <Filter size={18} />
                </button>
             </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8 flex items-center gap-2">
           ...
           <h2 className="text-xl font-bold text-white font-display">Top Traders to Copy</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <TraderProfileCard 
             rank={1} name="Satoshi_Ghost" pnl="412.5" winRate="78" risk={6} managed="$4.2M" followers="12k" tags={['Fast', 'Bitcoin']} 
          />
          <TraderProfileCard 
             rank={2} name="Alpha_Vector" pnl="389.2" winRate="65" risk={4} managed="$2.8M" followers="8.5k" tags={['Steady', 'Ethereum']} 
          />
          <TraderProfileCard 
             rank={3} name="Quantum_Leap" pnl="256.4" winRate="82" risk={3} managed="$8.1M" followers="15k" tags={['Safe', 'Long Term']} 
          />
          <TraderProfileCard 
             rank={4} name="Void_Runner" pnl="198.2" winRate="55" risk={8} managed="$1.2M" followers="3.2k" tags={['Aggressive', 'Solana']} 
          />
          <TraderProfileCard 
             rank={5} name="Iron_Bank" pnl="145.0" winRate="92" risk={2} managed="$15.4M" followers="22k" tags={['Very Safe', 'Stable']} 
          />
          <TraderProfileCard 
             rank={6} name="Neon_Blade" pnl="132.8" winRate="68" risk={5} managed="$980k" followers="4.1k" tags={['Daily', 'Mixed']} 
          />
          <TraderProfileCard 
             rank={7} name="Cipher_Punk" pnl="98.5" winRate="61" risk={7} managed="$650k" followers="2.8k" tags={['Trending']} 
          />
          <TraderProfileCard 
             rank={8} name="HODL_Gawd" pnl="87.2" winRate="45" risk={9} managed="$320k" followers="1.5k" tags={['High Risk']} 
          />
        </div>
        ...
        <div className="mt-16 p-8 rounded-2xl bg-gradient-to-r from-blue-900/20 to-transparent border border-blue-500/20 flex flex-col md:flex-row items-center justify-between gap-8">
           ...
           <button className="whitespace-nowrap px-8 py-3 bg-white text-[#030508] font-bold uppercase tracking-wider rounded hover:bg-gray-200 transition-colors">
              Set Safety Limits
           </button>
        </div>
      </div>
